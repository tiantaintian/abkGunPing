var FeedbackUtil = {
	feed:function(feedbackurl) {
		...
		stylehtml += '#feedback_popup h2, #feedback_popup h2 .close, .pop_box .button, .pop_box .button_fd, .loginlist li span, .pop_login li.cwts em{ } ';
		...
		stylehtml += '#feedback_popup h2{ background-position:left top; background-repeat:repeat-x; height:33px; line-height:33px;   padding-left:12px; color:#000; font-size:12px; font-weight:normal; overflow:hidden; width:560px;background: #00ccff;}';
		stylehtml += '#feedback_popup h2 .close { background-position:-90px -35px; cursor:pointer; float:right; width:12px; height:12px; margin:0px 10px 0 0; _display:inline; z-index:10; font-size:20px;}';
		...
		stylehtml += '#feedback_popup .pop_box .button{ background-position:0 -35px; float:left; width:85px; height:32px; cursor:pointer; border:0; font-size:14px; color:#fff; font-weight:bold; line-height:32px;background: #00ccff;}';

		stylehtml += '.pop_login{ height:auto; overflow:hidden; zoom:1; padding-left:70px; position:relative;}';
		stylehtml += '.pop_login li{ clear:both; height:42px; overflow:hidden; line-height:27px;zoom:1; color: #000;}';
		...
		stylehtml += '.pop_login li .r .input input, .pop_login #login_username, .pop_login #login_password, .pop_login #tcentry ul .zcinput, .pop_login #tcentry ul .yzinput, .pop_login #feedback_popup_main ul input{ width:188px; height:25px; line-height:25px; padding:0 5px; border:0; margin:1px 2px 0 0; font-size:12px;}';
		...
		html +='<h2><a href="javascript:void(0);" target="_self" onclick="FeedbackUtil.close();return false;" class="close">×</a>填写您的信息，免费通话沟通！</h2>';
		html +='<div class="pop_box">';
		html +='<!-- 登录框 s -->';
			html +='<ul class="pop_login" style="margin-left:5%;text-align:center;">';
				html +='<li><span class="l">姓名：</span>';
					html +='<span class="r">';
						html +='<span class="input"><input type="text" placeholder="输入您的姓名" tabindex="1" id="fb_email" name="realname"></span>';
						html +='<em class="t_right"></em><em class="t_wrong" id="email_wrong">邮箱不能为空<em></em></em>';
					html +='</span>';
				html +='</li>';
				html +='<li><span class="l">手机号：</span>';
					html +='<span class="r">';
						html +='<span class="input"><input type="text" placeholder="输入您的手机号" tabindex="2" id="fb_mobile" name="mobile"></span>';
						html +='<em class="t_right"></em><em class="t_wrong" id="mobile_wrong">邮箱不能为空<em></em></em>';
					html +='</span>';
				html +='</li>';
	...
	send:function(feedbackurl) {
		var realname = $('#fb_email').val();
		var mobile = $('#fb_mobile').val();
		if (realname == '' ) {
			$("#email_wrong").html('请填写您的姓名！').show();
			return '';
		}
		
		if (mobile == '' ) {
			$("#mobile_wrong").html('请填写您的手机号码！').show();
			return '';
		}
		...
		$.ajax({
			...
			success:function(msg){
				if(msg.status == 1){	 
					FeedbackUtil.tip('succ', msg.info);
					FeedbackUtil.close(); 
				}else{
					FeedbackUtil.tip('fail', msg.info);
					FeedbackUtil.close();
				}
			}
		});
